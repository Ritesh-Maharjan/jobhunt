...
import { getAllUser } from "../api/jobApi";
    ...
    <div className="flex">
      <div className="flex flex-col w-[90vw] m-auto items-center">
        <form className="flex items-center w-full relative m-2">
          ...
          />
          <button onClick={submitSearch} className="absolute right-0 p-2 text-center">
            <svg
              ...
              <Link to={`job/${el._id}`}>
                ...
                >
                  <div className="flex flex-col w-[10vw] justify-center items-center px-2 mr-6">
                    {el?.createdBy.avatar ? (
                      <img src={el?.createdBy.avatar} className="h-12" />
                    ) : (
                      <img src="https://imgs.search.brave.com/lQJ580-JievQJ14gi6KKJrwsK5Yln9K2ECOia6lOlBg/rs:fit:474:225:1/g:ce/aHR0cHM6Ly90c2U0/Lm1tLmJpbmcubmV0/L3RoP2lkPU9JUC5E/WkxXRnFZcUlHNGxf/eUphcU91SlhnSGFI/YSZwaWQ9QXBp" />
                    )}
                    <h1 className="text-3xl">{el?.createdBy?.name}</h1>
                  </div>
